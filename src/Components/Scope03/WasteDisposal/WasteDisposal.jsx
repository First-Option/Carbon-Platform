...
import "./WasteDisposal.css"
import Chart from 'chart.js/auto';

function WasteDisposal() {
  const [calculations, setCalculations] = useState(Array(39).fill(0));
  const [factors, setFactors] = useState(Array(39).fill(0));
  ...
  const labels = rowsData.map(row => row.label);
    ...
    localStorage.setItem('WasteDisposalData', JSON.stringify(savedData));
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
